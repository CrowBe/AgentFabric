...
4. Records ownership feedback for the checkout outside `.fabric/`. Every dirty git-tracked path is repo-owned. An untracked path is repo-owned when it matches a `repo_owned.include` pattern in `.agentfabric-sync.json`; anything else is machine-local harness state and stays silent. Without that manifest every untracked path is repo-owned; if it exists but cannot be read, or does not declare `repo_owned.include` as a non-empty list of patterns, sync falls back to that same treatment and names the manifest in the feedback.
