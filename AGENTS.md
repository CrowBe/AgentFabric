...
That is the first step. It installs the package if needed, initialises `.fabric/`, and verifies the **example resolved catalogue** (`workspace.discover`, `blob.read`, `blob.write`, `text.normalize`, plus example-domain `journal.append` / `journal.digest`). Those journal operations are demos, not AgentSOP primitives. Do not start from Bash.
